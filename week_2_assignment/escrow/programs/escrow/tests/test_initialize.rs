...
fn test_initialize() {
    ...
    let counter = Pubkey::find_program_address(&[escrow::constants::COUNTER_SEED], &program_id).0;
    ...
    let counter_state = escrow::state::Counter::try_deserialize(&mut data).unwrap();
    assert_eq!(counter_state.count, 0);
    assert_eq!(counter_state.authority, payer.pubkey());

    let instruction = Instruction::new_with_bytes(
        program_id,
        &escrow::instruction::Increment {}.data(),
        escrow::accounts::Increment {
            counter,
