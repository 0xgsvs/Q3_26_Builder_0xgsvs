...
describe("NFT devnet flow", () => {
  ...
  test("mintNft creates asset with Pixel_pic and reused metadata URI", async () => {
  ...
  test("updateNft changes name and uri as update authority", async () => {
    ...
    const {
      ...
      newName: retName,
      newUri: retUri,
    } = await updateNft(umi, assetPubkey, {
      newName,
      newUri,
    });
    expect(signature).toBeDefined();
  ...
  test("burn closes the account (Arweave URIs remain permanently — only on-chain burned)", async () => {
    expect(assetPubkey).toBeDefined();
