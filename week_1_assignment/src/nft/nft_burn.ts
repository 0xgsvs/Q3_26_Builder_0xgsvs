...
export async function burnAllForOwner(umi: Umi): Promise<string[]> {
  ...
  console.log(pc.yellow(`Found ${assets.length} assets for ${umi.identity.publicKey} — burning...`));
  const sigs: string[] = [];
  for (const a of assets) {
    const sig = await burnNft(umi, a.publicKey);
    sigs.push(sig);
  }
  console.log(pc.green(`Burned ${sigs.length} assets.`) + pc.dim(' Arweave URIs (gateway.irys.xyz/...) remain permanently — only on-chain AssetV1 closed.'));
  return sigs;
}
